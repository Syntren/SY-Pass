...
- **🖼️ Real Server Icons & Favicons:** Displays real server icons (`server.png` / favicon) from your vanilla multiplayer list (`servers.dat`) or active server connection directly beside each entry. Includes GPU texture caching and zero-overhead memory management.
...
- **✨ Quick & Smart Auto-Registration:** One-step registration on servers (`/sypass register [len]` or `/sypass quickreg`). Generates a cryptographically secure password, copies it to the clipboard, saves it locally, syncs to Bitwarden, and submits the `/register <password> <password>` command. Configurable default password length (6–64 chars) with quick stepper controls (`[-] [16] [+]`) in the GUI. Includes built-in **Overwrite Protection** to prevent accidental loss of existing credentials.
